...
3) Copy env template and fill in IDs/tokens:
	 - `cp .env.example .env`
4) Place your Google OAuth client secret JSON in `secrets/client_secret.json`.
	 - The app will migrate/standardize from other locations if found.
5) Authenticate once (opens browser):
	 - `python -m src`
...
- Initialize project structure:
	...
	Note: Environment variables override values in .env (env wins). .env is optional but convenient for local dev.
	...
	- Module entrypoint (recommended):
		- python -m src --help
	- Show effective/default/canary API versions:
		- python -m src --show-version
	- List CAP alerts, rules dry-run, queue ops, region mappings:
		- python -m src --caps --caps-limit 5
		- python -m src --rules-dry-run --rules-file rules/example.yaml
		- python -m src --queue-status
		- python -m src --list-regions
